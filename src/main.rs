...
impl App {
    ...
    fn get_total_from(self: &Self, c: &Connection, from: &DateTime<Utc>) -> Duration {
    ...
    fn print_resume(self: &Self, c: &Connection) {
        // Print worked time
        let now = Utc::now();

        let begin_of_day = now.beginning_of_day();
        let day_total = self.get_total_from(c, &begin_of_day);
        println!(
        ...
        if begin_of_day != begin_of_week {
            let week_total = self.get_total_from(c, &begin_of_week);
            println!(
    ...
    fn open_db(self: &Self) -> Result<sqlite::Connection> {
        let db_file = std::path::Path::new(&self.work_db);
    ...
    fn do_checkin(self: &Self) -> Result<()> {
        let c = self.open_db().context("Opening database")?;
...
fn main() -> Result<()> {
    ...
    let app = App::new("prod.sqlite".to_string());
